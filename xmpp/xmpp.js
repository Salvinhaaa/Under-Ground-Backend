...
if (config.bEnableHTTPS) {
    ...
    httpsServer.listen(port, () => {
        log.xmpp(`XMPP e Matchmaker na porta ${port} (SSL Enabled)`);
    });
} else {
    wss = new WebSocket({ server: app.listen(port) });
    log.xmpp(`XMPP e Matchmaker na porta ${port} (SSL Disabled)`);
}
...
function RemoveClient(ws, joinedMUCs) {
    ...
    log.xmpp(`O ${client.displayName} Saiu.`);
}
