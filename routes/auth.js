    ...
    if (config.bEnableOnlyOneVersionJoinable === true) {
        if (memory.build != config.bVersionJoinable) {
            ...
            return error.createError(
                ...
                "This version is blocked, change bSeasonJoinable or disable bEnableOnlyOneSeasonJoinable on Under Ground config.",
                [], -1, undefined, 400, res
            );
        }
    }
...
app.get("/account/api/oauth/verify", verifyToken, (req, res) => {
    ...
    if (config.bEnableOnlyOneVersionJoinable === true) {
        if (memory.build != config.bVersionJoinable) {
            ...
            return error.createError(
                "errors.com.epicgames.version_not_supported",
                "This version is no longer supported, change bSeasonJoinable or disable bEnableOnlyOneSeasonJoinable on Under Ground config.",
                [], -1, undefined, 400, res
            );
        }
    }
