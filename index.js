...
console.log('Under Ground                                                 \x1b[36mTodos os creditos para @.salvinhaa. e @santi.2._ no discord\n');
...
if (!packageJson) throw new Error("Falha na leitura de package.json");
...
mongoose.connect(config.mongodb.database, () => {
    log.backend("Backend conectado com a database!");
});

mongoose.connection.on("error", err => {
    log.error("a conexão com a database falhou");
    throw err;
});
...
fs.readdirSync("./routes").forEach(fileName => {
    try {
        ...
        log.error(`Routes Error: Falha a carregar ${fileName}`)
    }
});
...
fs.readdirSync("./Api").forEach(fileName => {
    try {
        ...
        log.error(`UnderGround API Error: Falha a carregar ${fileName}`)
    }
});

app.get("/unknown", (req, res) => {
    log.debug('GET /unknown endpoint called');
    res.json({ msg: "UnderGround - Made by zynix and salva" });
});

let server;
if (config.bEnableHTTPS) {
    server = httpsServer.listen(PORT, () => {
        log.backend(`Backend conectado a porta ${PORT} (SSL Enabled)`);
        ...
        if (err.code === "EADDRINUSE") {
            log.error(`Porta ${PORT} esta a ser usado!\na fechar em 3sec...`);
    ...
    server = app.listen(PORT, () => {
        log.backend(`Backend conectado a porta${PORT} (SSL Disabled)`);
        require("./xmpp/xmpp.js");
        if (config.discord.bUseDiscordBot === true) {
            require("./DiscordBot");
        }
        ...
        if (err.code === "EADDRINUSE") {
            log.error(`Port ${PORT} esta a ser usado!\na fechar em 3sec...`);
            await functions.sleep(3000);
            process.exit(0);
        } else {
            throw err;
...
if (config.bEnableCalderaService === true) {
    ...
    if (config.bEnableHTTPS) {
        ...
        if (!config.bGameVersion) {
            log.calderaservice("Difene a versao nas settings!")
            return;
        }

        calderaHttpsServer.listen(config.bCalderaServicePort, () => {
            log.calderaservice(`Caldera esta conectado a porta ${config.bCalderaServicePort} (SSL Enabled)`);
        }).on("error", async (err) => {
            if (err.code === "EADDRINUSE") {
                log.calderaservice(`Caldera serviço de porta ${config.bCalderaServicePort}  esta a ser usado!\na fechar em 3sec...`);
                await functions.sleep(3000);
                process.exit(1);
            } else {
                throw err;
        ...
        if (!config.bGameVersion) {
            log.calderaservice("Difene nas settings!")
            return;
        }

        calderaService.listen(config.bCalderaServicePort, () => {
            log.calderaservice(`Caldera esta nporta ${config.bCalderaServicePort} (SSL Disabled)`);
        }).on("error", async (err) => {
            if (err.code === "EADDRINUSE") {
                log.calderaservice(`Caldera serviço de port ${config.bCalderaServicePort} esta a ser usado!\na fechar em 3sec...`);
                await functions.sleep(3000);
                process.exit(1);
            } else {
                throw err;
...
if (config.Website.bUseWebsite === true) {
    ...
    if (config.bEnableHTTPS) {
        ...
        httpsServer.listen(config.Website.websiteport, () => {
            log.website(`Website na ${config.Website.websiteport} (SSL Enabled)`);
        }).on("error", async (err) => {
            if (err.code === "EADDRINUSE") {
                log.error(`Website porta ${config.Website.websiteport} esta a ser usado!\na fechar em 3sec...`);
                await functions.sleep(3000);
                process.exit(1);
            } else {
                throw err;
        ...
        websiteApp.listen(config.Website.websiteport, () => {
            log.website(`Website na ${config.Website.websiteport} (SSL Disabled)`);
        }).on("error", async (err) => {
            if (err.code === "EADDRINUSE") {
                log.error(`Website porta ${config.Website.websiteport} esta a ser usado!\na fechar em 3sec...`);
                await functions.sleep(3000);
...
app.use((req, res, next) => {
    ...
    log.debug(`Missing endpoint: ${req.method} ${url} pedir porta ${req.socket.localPort}`);
    if (req.url.includes("..")) {
